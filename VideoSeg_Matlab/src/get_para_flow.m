...
para.p=p;
para.q=q;
para.alpha = 70;  % weight of smoothness.
% para.beta = 300;    % weight for descriptor positions match
% para.alpha = 250;  % weight of smoothness.
para.beta = 300;    % weight for descriptor positions match
...
para.level = ceil((log(60)-log(para.q))/log(para.downsampling));            % levels of pyramid.:we need 70 for very large displacements
para.w = 1.99;
...
para.cellsize=3;  %half size of a HOG descriptor
...
para.nbins=1;       %how many descriptors to aggregate in each direction
